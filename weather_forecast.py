...
- Kein Token-Leak über Redirects (allow_redirects=False).
"""
...
def get_credentials() -> Tuple[str, str]:
    ...
    diese Datei liest NUR systemd (root), nicht dieses Skript.
    """
    client_id = os.environ.get("SRG_CLIENT_ID")
    client_secret = os.environ.get("SRG_CLIENT_SECRET")

    if not client_id or not client_secret:
        home_env_path = os.path.expanduser("~/.srg-meteo.env")
        try:
            env = load_env_file_secure(home_env_path)
            client_id = client_id or env.get("SRG_CLIENT_ID")
            client_secret = client_secret or env.get("SRG_CLIENT_SECRET")
        except ApiError as e:
            # Expliziter Hinweis, falls die Datei existiert, aber unsicher ist
    ...
    if not client_id or not client_secret:
        msg = (
            ...
            "Hinweis: Für einen systemd-Betrieb nutze ein root-only /etc/srf-meteo.env und EnvironmentFile= (siehe Anleitung)."
        )
...
def get_access_token(client_id: str, client_secret: str) -> str:
    """OAuth2 Client-Credentials – Token via Basic Auth + grant_type im Body."""
    ...
    r = requests.post(OAUTH_TOKEN_URL, headers=headers, data=data, timeout=20, allow_redirects=False)
    if not r.ok:
